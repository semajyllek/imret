...
Comparison against OpenCV `BFMatcher` (exact Hamming, same ORB features) and `imagehash` (perceptual hashing) at increasing collection sizes. Generated by `benchmark.py` on macOS arm64, `max_features=500`.

`imret` and `imret-batch` are identical at search time; the batch variant uses `add_batch()` which extracts ORB features in parallel via OpenMP, reducing build time.

**Search latency p50 (ms)** — lower is better

| N | imret | imret-batch | bfmatcher | imagehash |
|---|---|---|---|---|
| 100 | 2.97 | 3.00 | 8.71 | 0.21 |
| 500 | 3.38 | 3.31 | 38.80 | 0.52 |
| 1,000 | 3.30 | 3.26 | 83.85 | 0.89 |
| 2,000 | 3.48 | 3.38 | 171.27 | 1.74 |

**Accuracy (%)** — re-query test, higher is better
...
| 2,000 | 100.0 | 100.0 | 100.0 | 100.0 |

**Build time (s)**

| N | imret | imret-batch | bfmatcher | imagehash |
|---|---|---|---|---|
| 100 | 1.09 | 0.96 | 0.11 | 1.97 |
| 500 | 15.82 | 15.38 | 0.56 | 0.08 |
| 1,000 | 31.68 | 30.50 | 1.12 | 0.15 |
| 2,000 | 63.40 | 60.82 | 2.23 | 0.37 |

imret's search latency is nearly flat (2.97ms → 3.48ms across 20× more images) while BFMatcher grows linearly with collection size. Build time is higher because imret trains a FAISS IVF index upfront — this is a one-time cost paid once at index time, not at query time.

The imagehash accuracy of 100% is an artifact of the synthetic test images being structurally distinct. On real-world collections with visually similar images (e.g., paintings by the same artist), perceptual hashing degrades significantly. Run `python benchmark.py --plot` to reproduce and generate `benchmark_results.png`.
